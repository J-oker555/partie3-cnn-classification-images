...
from cnn_image_classification.models import build_cnn_augmented, build_data_augmentation
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the augmented CNN with Dropout and save diagnostics.")
...
def main() -> None:
    ...
    history, training_time = fit_with_timing(
        ...
        epochs=args.epochs,
        callbacks=[
            tensorboard_callback(args.logs_dir, "augmented"),
            early_stopping_loss(),
        ],
    )
    ...
    plot_history(history, "CNN augmente Dropout", args.outputs_dir / "curves_cnn_augmente_dropout.png")
    model.save(args.models_dir / "model_augmented.keras")
    save_run_summary(
        args.outputs_dir / "history_augmented.json",
        history=history,
